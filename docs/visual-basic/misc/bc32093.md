...
ステートメントにより、[Of](../../visual-basic/language-reference/statements/of-clause.md) 句を使用せずに、ジェネリック型からの型の構築またはジェネリック メソッドの呼び出しが試行されます。  
 ...
 ジェネリック型用の [!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)] 構文には、`Of` キーワードによって導入される型パラメーターと型引数が必要です。 さらに、型パラメーター リストまたは型引数リストは、かっこで囲まれている必要があります。  
