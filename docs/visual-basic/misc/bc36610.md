...
1.  参照元のステートメントで名前のスペルを確認します。[!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] は、大文字と小文字を区別しませんが、スペルにその他の違いがあった場合には異なる名前であると見なします。 アンダースコア \(`_`\) も名前の一部であり、スペルに含まれます。  
