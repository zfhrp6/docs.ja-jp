...
メンバー '\<membername1\>' は暗黙的に '\<implicitmembername\>' を宣言し、基底クラス '\<baseclassname\>' のメンバー '\<membername2\>' に対して暗黙的に宣言されているメンバーと競合します。 そのため、メンバーは 'Overloads' と宣言できません。  
 ...
 [!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] コンパイラは、宣言されている特定のプログラミング要素に対応する暗黙的なメンバーを作成します。 次の表には、これらの暗黙なメンバー、つまり*統合*メンバーをまとめています。  
