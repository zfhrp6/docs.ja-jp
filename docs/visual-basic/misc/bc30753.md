...
`Option Strict On` では `<type1>` から `<type2>` への暗黙的な変換は許可されていません。Visual Basic 6.0 のコレクション型は [!INCLUDE[dnprdnshort](../../csharp/getting-started/includes/dnprdnshort_md.md)] のコレクション型と互換性がありません。  
  
 Visual Basic 6.0 で使用されるコレクション オブジェクトが、[!INCLUDE[vs_current_long](../../csharp/misc/includes/vs_current_long_md.md)] で使用されているコレクション オブジェクトと異なります。  
