...
オーバーロードされたバージョンが複数定義されているプロシージャが呼び出された場合、[!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)] のコンパイラは、どのオーバーロードを呼び出すのかを判断する必要があります。  この判断は、次の手順で行われます。  
