...
[!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)] では、条件を調べ、その結果に応じた処理を実行できます。  条件のテスト結果は、真偽値、式のさまざまな値、または一連のステートメントを実行したときに生成されるさまざまな例外として得ることができます。  
