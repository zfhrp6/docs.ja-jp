...
-   多重継承をサポートしている言語もありますが、[!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] がサポートしているのは単一継承だけです。したがって、派生クラスの基本クラスは常に 1 つだけです。  クラスでは多重継承することはできませんが、複数のインターフェイスを実装することはできます。複数のインターフェイスを実装すると、多重継承と同様の結果が得られます。  
...
## 継承の修飾子  
 [!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] では、継承をサポートするために、次に示すクラス レベルのステートメントと修飾子が導入されています。  
