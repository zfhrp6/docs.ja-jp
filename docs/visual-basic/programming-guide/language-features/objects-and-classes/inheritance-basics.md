...
-   複数の継承を許可する言語とは異なり[!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]クラス内の 1 つだけの継承を許可は、派生クラスが 1 つだけの基本クラスを持つことができます。 クラスに複数の継承は許可されていませんが、クラスは、同じ結果を達成できる効果的に複数のインターフェイスを実装できます。  
...
## <a name="inheritance-modifiers"></a>継承の修飾子  
 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]次のクラス レベルのステートメントおよび継承をサポートする修飾子が導入されています。  
