...
## 文字リテラルがコンパイル エラーを発生させる  
 型宣言文字がない場合、[!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)] ではデータ型がリテラルであると見なされます。  文字リテラル \(二重引用符 \(`" "`\) で囲まれた文字\) の既定の型は、`String` です。  
