...
-   コード エディターで **\[コードの再フォーマット\]** を使用してコードの書式を再設定します。  詳細については、「[\[オプション\]、\[テキスト エディター\]、\[基本\] \(Visual Basic\)](../Topic/Options,%20Text%20Editor,%20Basic%20\(Visual%20Basic\).md)」を参照してください。  
