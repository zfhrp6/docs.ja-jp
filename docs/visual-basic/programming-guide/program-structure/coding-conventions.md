...
-   名前付けのガイドラインについては、次を参照してください。[名前付けのガイドライン](http://msdn.microsoft.com/library/fc076d66-9b5f-42d3-aa65-61d970c794a3)トピックです。  
...
-   使用**(の再フォーマット コード再**コード エディターで、コードの書式設定を変更します。 詳細については、次を参照してください。[オプション、テキスト エディター]、[基本 (Visual Basic)](https://docs.microsoft.com/visualstudio/ide/reference/options-text-editor-basic-visual-basic)します。  
...
-   場合**(の再フォーマット コード再**しない形式継続行に自動的に、手動でインデント継続行&1; タブ ストップ。 ただし、リストの項目は常に左揃えにします。  
...
-   ループ内で文字列を追加するには、使用、<xref:System.Text.StringBuilder>オブジェクト</xref:System.Text.StringBuilder>。  
