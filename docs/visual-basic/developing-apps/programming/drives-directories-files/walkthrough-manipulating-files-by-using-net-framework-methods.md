...
## アプリケーションの作成  
 [!INCLUDE[vsprvs](../../../../csharp/includes/vsprvs-md.md)] を起動し、ユーザーが指定のファイルに書き込みを行うためのフォームを作成して、プロジェクトを開始します。  
...
3.  **\[プロジェクト名\]** ボックスに「`MyDiary`」と入力し、**\[OK\]** をクリックします。  
     ...
     [!INCLUDE[vsprvs](../../../../csharp/includes/vsprvs-md.md)] によってプロジェクトが**ソリューション エクスプローラー**に追加され、**Windows フォーム デザイナー**が表示されます。  
