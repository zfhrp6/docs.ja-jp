...
## <a name="permissions"></a>アクセス許可  
 ...
 レジストリを操作すると、システム リソースや保護情報への不適切なアクセスが許可され、セキュリティが損なわれる場合があります。 これらのプロパティを使うには、<xref:System.Security.Permissions.RegistryPermissionAccess> 列挙型の読み書きアクセス許可が必要です。これは、レジストリ変数へのアクセスを制御します。 完全な信頼で実行されるコード (既定のセキュリティ ポリシーでは、これはユーザーのローカル ハード ディスクにインストールされているコードです) は、レジストリにアクセスするために必要なアクセス許可を持っています。 詳しくは、<xref:System.Security.Permissions.RegistryPermission> クラスをご覧ください。  
 ...
 レジストリ アクセス許可のアクセスの値は、<xref:System.Security.Permissions.RegistryPermissionAccess> 列挙型によって定義されています。 次の表はそのメンバーの詳細です。  
