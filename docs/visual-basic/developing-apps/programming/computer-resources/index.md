...
`My.Computer` オブジェクトは、`My` の中心となる 3 つのオブジェクトのうちの 1 つであり、情報へのアクセスと共通で使用される機能へのアクセスを提供します。  `My.Computer` には、アプリケーションを実行中のコンピューターにアクセスするためのメソッド、プロパティ、およびイベントが用意されています。  含まれているオブジェクトは次のとおりです。  
...
## このセクションの内容  
 ...
 コンピューターの完全名や IP アドレスの判断など、`My.Computer.Info` に関連するタスクを一覧表示します。  
