...
-   ステートメントに `exception` 変数がない \(例: `Catch`\)。  
 ...
 `Try…Catch…Finally` ステートメントが別の `Try` ブロック内に入れ子になっている場合、[!INCLUDE[vbprvb](../../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] は、最初に最も内側の `Try` ブロック内の各 `Catch` ステートメントをチェックします。  対応する `Catch` ステートメントが見つからなかった場合は、外側にある `Try…Catch…Finally` ブロックの `Catch` ステートメントが検索されます。  
