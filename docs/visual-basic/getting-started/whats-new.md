...
## 現在のバージョン  
 ...
 文字列をハードコーディングせずにエラー メッセージで使用するための型またはメンバーの非修飾文字列名を取得できます。  これにより、リファクタリングするときにコードを適切な状態に保つことができます。  この機能は、またモデル\-ビュー\-コント ローラーの MVC のリンクをフックし、プロパティ変更イベントを発生させるためにも役立ちます。  
  
 [文字列の補間](../../csharp/language-reference/keywords/interpolated-strings.md)  
 文字列の補間の式を使用して、文字列を構築することができます。  補間の文字列式は、式が含まれているテンプレート文字列のように見えます。  C\# は、式を式の結果の ToString 表現に置き換えることで文字列を作成します。  引数に関しては、補間文字列は[複合書式指定](../Topic/Composite%20Formatting.md)より理解しやすいです。  
  
 [Null 条件付きのメンバー アクセスとインデックス作成](../../csharp/language-reference/operators/null-conditional-operators.md)  
 メンバー アクセス \(`?.`\) またはインデックス \(`?[]`\) 操作を実行する前に、構文的に非常に簡単な方法で null をテストできます。  これらの演算子では、null チェックの処理のために記述するコードを少なくすることができます \(特に、データ構造に降りる場合\)。  左のオペランドまたはオブジェクト参照が null の場合、操作は null を返します。  
