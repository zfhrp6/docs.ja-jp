...
|ColumnName|DataType|説明|  
|----------------|--------------|-----------------|  
|CONSTRAINT_CATALOG|文字列型|制約が属するカタログ。|  
|CONSTRAINT_SCHEMA|文字列型|制約を含むスキーマ。|  
|CONSTRAINT_NAME|文字列型|名前。|  
|TABLE_CATALOG|文字列型|制約が含まれるテーブル名。|  
|TABLE_SCHEMA|文字列型|テーブルを含むスキーマ。|  
|TABLE_NAME|文字列型|テーブル名|  
|CONSTRAINT_TYPE|文字列型|制約の型。 "FOREIGN KEY" だけが許可されています。|  
|IS_DEFERRABLE|文字列型|制約を遅延可能にするかどうかを指定します。 NO が返されます。|  
|INITIALLY_DEFERRED|文字列型|制約を最初に遅延可能にするかどうかを指定します。 NO が返されます。|  
...
|ColumnName|DataType|説明|  
|----------------|--------------|-----------------|  
|type_desc|String|インデックスの種類。次のいずれかの値になります。<br /><br /> -HEAP<br />クラスター化<br />非クラスター化<br />XML<br />空間|  
...
|ColumnName|DataType|説明|  
|----------------|--------------|-----------------|  
|SPECIFIC_CATALOG|文字列型|カタログ固有の名前。|  
|SPECIFIC_SCHEMA|文字列型|スキーマ固有の名前。|  
|SPECIFIC_NAME|文字列型|カタログ固有の名前。|  
|ROUTINE_CATALOG|文字列型|ストアド プロシージャが属するカタログ。|  
|ROUTINE_SCHEMA|文字列型|ストアド プロシージャを含むスキーマ。|  
|ROUTINE_NAME|文字列型|ストアド プロシージャの名前。|  
|ROUTINE_TYPE|文字列型|ストアド プロシージャには PROCEDURE が返され、関数には FUNCTION が返されます。|  
...
|ColumnName|DataType|説明|  
|----------------|--------------|-----------------|  
|SPECIFIC_CATALOG|文字列型|このパラメーターを受け取るプロシージャのカタログ名。|  
|SPECIFIC_SCHEMA|文字列型|このパラメーターを受け取るプロシージャを含むスキーマ。|  
|SPECIFIC_NAME|文字列型|このパラメーターを受け取るとるプロシージャ名。|  
|ORDINAL_POSITION|Int32|パラメーターの位置を示す 1 から始まる序数。 プロシージャの戻り値については 0 になります。|  
|PARAMETER_MODE|文字列型|入力パラメーターでは IN が返され、出力パラメーターでは OUT が返され、I/O パラメーターでは INOUT が返されます。|  
|IS_RESULT|文字列型|プロシージャの結果が関数を表す場合には YES が返されます。 その他の場合は NO が返されます。|  
|AS_LOCATOR|文字列型|ロケーターとして宣言された場合は YES が返されます。 その他の場合は NO が返されます。|  
|PARAMETER_NAME|文字列型|パラメーターの名前。 関数の戻り値に相当する場合は NULL になります。|  
|DATA_TYPE|文字列型|システムにより提供されるデータ型。|  
...
|COLLATION_CATALOG|文字列型|パラメーター照合のカタログ名。 文字型の 1 つでない場合は、NULL が返されます。|  
|COLLATION_SCHEMA|文字列型|常に NULL が返されます。|  
|COLLATION_NAME|文字列型|パラメーター照合の名前。 文字型の 1 つでない場合は、NULL が返されます。|  
|CHARACTER_SET_CATALOG|文字列型|パラメーターの文字セットのカタログ名。 文字型の 1 つでない場合は、NULL が返されます。|  
|CHARACTER_SET_SCHEMA|文字列型|常に NULL が返されます。|  
|CHARACTER_SET_NAME|文字列型|パラメーターの文字セット名。 文字型の 1 つでない場合は、NULL が返されます。|  
...
|INTERVAL_TYPE|文字列型|NULL。 将来 SQL Server で使用するために予約されています。|  
...
|ColumnName|DataType|説明|  
|----------------|--------------|-----------------|  
|TABLE_CATALOG|文字列型|テーブルのカタログ。|  
|TABLE_SCHEMA|文字列型|テーブルを含むスキーマ。|  
|TABLE_NAME|文字列型|テーブル名。|  
|TABLE_TYPE|文字列型|テーブルの型。 VIEW または BASE TABLE のいずれかです。|  
...
|COLUMN_DEFAULT|文字列型|列の既定値。|  
|IS_NULLABLE|文字列型|列に NULL 値が許容されるかどうかを指定します。 この列に NULL が許容される場合は、YES が返されます。 その他の場合は NO が返されます。|  
|DATA_TYPE|文字列型|システムにより提供されるデータ型。|  
...
|CHARACTER_SET_CATALOG|文字列型|列が文字データ型またはテキスト データ型である場合は、文字セットがあるデータベースを示すマスターが返されます。 その他の場合は NULL が返されます。|  
|CHARACTER_SET_SCHEMA|文字列型|常に NULL が返されます。|  
|CHARACTER_SET_NAME|文字列型|この列が文字データ型またはテキスト データ型である場合、文字セットの一意の名前が返されます。 その他の場合は NULL が返されます。|  
|COLLATION_CATALOG|文字列型|列が文字データ型またはテキスト データ型である場合は、照合順序が定義されているデータベースを示すマスターが返されます。 その他の場合、この列は NULL になります。|  
...
|user_name|文字列型|このデータベースで一意のユーザー名またはグループ名。|  
...
|ColumnName|DataType|説明|  
|----------------|--------------|-----------------|  
|TABLE_CATALOG|文字列型|ビューのカタログ。|  
|TABLE_SCHEMA|文字列型|ビューを含むスキーマ。|  
|TABLE_NAME|文字列型|ビューの名前。|  
|CHECK_OPTION|文字列型|WITH CHECK OPTION の型。 元のビューが WITH CHECK OPTION を使用して作成されている場合は CASCADE になります。 その他の場合は NONE が返されます。|  
|IS_UPDATABLE|文字列型|ビューが更新可能であるかどうかを指定します。 常に NO が返されます。|  
  
## <a name="viewcolumns"></a>ViewColumns  
  
|ColumnName|DataType|説明|  
|----------------|--------------|-----------------|  
|VIEW_CATALOG|文字列型|ビューのカタログ。|  
|VIEW_SCHEMA|文字列型|ビューを含むスキーマ。|  
|VIEW_NAME|文字列型|ビューの名前。|  
|TABLE_CATALOG|文字列型|このビューに関連付けられているテーブルのカタログ。|  
|TABLE_SCHEMA|文字列型|このビューに関連付けられているテーブルを含むスキーマ。|  
|TABLE_NAME|文字列型|ビューに関連付けられているテーブルの名前。 ベース テーブルになります。|  
|COLUMN_NAME|String|列名。|  
...
|Permission_set_desc|文字列型|アセンブリのアクセス許可セット/セキュリティ レベルのフレンドリ名。|  
