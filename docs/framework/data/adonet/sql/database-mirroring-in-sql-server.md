---
title: "SQL Server のデータベース ミラーリング | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## 現在のサーバー名の取得  
 フェールオーバーの場合には、<xref:System.Data.SqlClient.SqlConnection> オブジェクトの <xref:System.Data.SqlClient.SqlConnection.DataSource%2A> プロパティを使って、現在の接続が実際に接続されているサーバーの名前を取得できます。  次のコード フラグメントでは、アクティブなサーバーの名前を取得します。この場合、接続変数は開いている <xref:System.Data.SqlClient.SqlConnection> を参照することを前提としています。  
...
|リソース|説明|  
|----------|--------|  
|SQL Server オンライン ブックの「[データベース ミラーリング](http://msdn.microsoft.com/library/bb934127.aspx)」|SQL Server でのミラーリングの設定と構成方法について説明します。|  
