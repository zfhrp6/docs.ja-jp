...
> [!IMPORTANT]
>  できるだけ Windows 認証を使用することをお勧めします。 Windows 認証では、暗号化された一連のメッセージを使用して SQL Server のユーザーを認証します。 SQL Server ログインを使用している場合、SQL Server ログイン名と暗号化されたパスワードが安全性が低い、ネットワーク経由で渡されます。  
