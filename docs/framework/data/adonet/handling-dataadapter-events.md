---
title: "DataAdapter のイベント処理 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
|`SkipAllRemainingRows`|更新操作を中止しますが、例外はスローしません。|  
 ...
 イベント ハンドラーを追加および削除する方法を次のコード サンプルに示します。  `RowUpdating` イベント ハンドラーは、削除されたすべてのレコードのログをタイムスタンプと共に記録します。  `RowUpdated` イベント ハンドラーは、`DataSet` の行の `RowError` プロパティにエラー情報を追加し、例外をスローせずに処理を続行します \(`ContinueUpdateOnError` \= `true` の場合と同等の動作です\)。  
...
|`Continue`|例外をスローするかどうかを選択できます。  `Continue` プロパティを `false` に設定すると、エラーが発生したときに現在の `Fill` 操作を停止し、例外をスローします。  `Continue` を `true` に設定すると、エラーに関係なく `Fill` 操作を続行します。|  
 ...
 次のコード例では、`DataAdapter` の `FillError` イベントにイベント ハンドラーを追加しています。  この `FillError` イベント コードの例は、有効桁の消失が発生したかどうかを確認し、例外に応答する機会を与えます。  
