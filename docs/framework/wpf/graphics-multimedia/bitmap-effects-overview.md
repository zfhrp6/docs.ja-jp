...
ビットマップ効果は、デザイナーを有効にして、視覚効果を適用する開発者が Windows Presentation Foundation (WPF) をコンテンツのレンダリングします。 たとえば、ビットマップ効果を簡単に適用できます、<xref:System.Windows.Media.Effects.DropShadowBitmapEffect>特殊効果またはイメージやボタンをぼかし効果。  
