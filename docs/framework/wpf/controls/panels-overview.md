...
#### <a name="virtualizingstackpanel"></a>VirtualizingStackPanel  
 ...
 次のマークアップを使用する方法を示しています、<xref:System.Windows.Controls.VirtualizingStackPanel>項目ホストとして。 <xref:System.Windows.Controls.VirtualizingStackPanel.IsVirtualizingProperty%2A?displayProperty=nameWithType>に添付プロパティを設定する必要があります`true`が発生する仮想化のためには、(既定)。  
