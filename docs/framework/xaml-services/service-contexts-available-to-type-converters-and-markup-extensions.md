 ...
 **関連:** <xref:System.IServiceProvider.GetService%2A?displayProperty=fullName> を呼び出すための、.NET Framework でのサービス ベースのインフラストラクチャの基本的な操作。  
 ...
 **サービス API:** 読み込みパスの場合は <xref:System.Xaml.IXamlNamespaceResolver.GetNamespace%2A>、保存パスの場合は <xref:System.Xaml.IXamlNamespaceResolver.GetNamespacePrefixes%2A>。  
