---
title: "相互運用可能なオブジェクト参照 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
```  
 ...
 ただし、`preserveObjectReferences` プロパティが `true` に設定されている場合でも、<xref:System.Runtime.Serialization.XsdDataContractExporter> がそのスキーマに `id` 属性や `ref` 属性を記述することはありません。  
  
## IsReference の使用  
 スキーマの記述に従った有効なオブジェクト参照情報を生成するには、<xref:System.Runtime.Serialization.DataContractAttribute> 属性を型に適用し、<xref:System.Runtime.Serialization.DataContractAttribute.IsReference%2A> フラグを `true` に設定します。  前のサンプル クラス `X` で `IsReference` を使用するには、次のようにします。  
