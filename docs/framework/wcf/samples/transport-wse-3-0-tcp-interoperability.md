---
title: "トランスポート : WSE 3.0 TCP 相互運用性 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
5.  チャネル スタックにカスタム トランスポートを追加するバインド要素を追加します。[!INCLUDE[crdefault](../../../../includes/crdefault-md.md)]「[バインド要素の追加](#AddingABindingElement)」を参照してください。  
  
## IDuplexSessionChannel の作成  
 WSE 3.0 TCP 相互運用性トランスポートを作成するには、まず、<xref:System.Net.Sockets.Socket> 上に <xref:System.ServiceModel.Channels.IDuplexSessionChannel> の実装を作成します。`WseTcpDuplexSessionChannel` は、<xref:System.ServiceModel.Channels.ChannelBase> から派生します。メッセージ送信のロジックは、\(1\) メッセージをバイトにエンコードし、\(2\) それらのバイトをフレーム化してネットワーク上に送信するという、2 つの主要部分で構成されます。  
 ...
 `WseTcpDuplexSessionChannel` は、<xref:System.ServiceModel.Channels.Message> と byte\[\] を相互に変換するために、<xref:System.ServiceModel.Channels.MessageEncoder> を使用します。`WseTcpDuplexSessionChannel` はトランスポートであるため、チャネルが構成されたリモート アドレスの適用も行います。`EncodeMessage` は、この変換ロジックをカプセル化します。  
  
 `this.RemoteAddress.ApplyTo(message);`  
  
 `return encoder.WriteMessage(message, maxBufferSize, bufferManager);`  
  
 <xref:System.ServiceModel.Channels.Message> がバイトにエンコードされたら、ネットワーク上に送信する必要があります。これを行うには、メッセージ境界を定義するシステムが必要です。WSE 3.0 は [DIME](http://go.microsoft.com/fwlink/?LinkId=94999) のバージョンをフレーム プロトコルとして使用します。`WriteData` はこのフレーム ロジックをカプセル化して、byte\[\] を一連の DIME レコードにラップします。  
...
-   `ClientWseTcpDuplexSessionChannel` は基本 `WseTcpDuplexSessionChannel`にロジックを追加して、`channel.Open` のときに TCP サーバーに接続します。まず、次のコードに示すようにホスト名を解決して IP アドレスに変換します。  
...
-   チャネル コントラクトの一部として、<xref:System.ServiceModel.CommunicationException> の `SocketException` など、ドメイン固有の任意の例外をラップします。  
  
## チャネル リスナー  
 TCP トランスポートを記述する次の手順では、サーバー チャネルを受け入れるための <xref:System.ServiceModel.Channels.IChannelListener> の実装を作成します。  
  
-   `WseTcpChannelListener` は <xref:System.ServiceModel.Channels.ChannelListenerBase>\<IDuplexSessionChannel\> から派生し、On\[Begin\]Open と On\[Begin\]Close をオーバーライドしてリッスン ソケットの有効期間を制御します。OnOpen で、IP\_ANY でリッスンするソケットを作成します。より高度な実装では、同様に IPv6 でリッスンする 2 つ目のソケットを作成できます。そのような実装では、IP アドレスをホスト名で指定することもできます。  
...
1.  このサンプルを実行するには、WSE 3.0 と WSE `TcpSyncStockService` サンプルがインストールされている必要があります。WSE 3.0 は、[MSDN](http://go.microsoft.com/fwlink/?LinkId=95000) からダウンロードできます。  
