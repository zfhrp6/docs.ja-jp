...
## Client  
 サンプルでは、<xref:System.ServiceModel.Discovery.DiscoveryClient> で <xref:System.ServiceModel.Discovery.DiscoveryClient.Find%2A> メソッドが呼び出されます。  次に、生成された <xref:System.ServiceModel.Discovery.FindResponse> に対して適切かつ必要な XML 要素が照会されます。  その後、これらの要素がコンソールに出力されます。  
