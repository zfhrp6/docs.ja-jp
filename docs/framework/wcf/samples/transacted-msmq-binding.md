...
> [!NOTE]
>  <xref:System.Messaging> を使用してキューを作成する場合、キュー名では、ローカル コンピューターを表すのにドット (.) を使用し、パスの区切りにはバックスラッシュを使用します。 Windows Communication Foundation (WCF) エンドポイント キュー アドレス net.msmq スキームを使用するには、"localhost"をローカル コンピューターを表すために使用およびパスはスラッシュを使用します。  
