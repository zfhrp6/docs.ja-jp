---
title: "Net.TCP ポート共有のサンプル | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
>  `<InstallDrive>:\WF_WCF_Samples\WCF\Basic\Binding\Net\TCP\PortSharing`  
 ...
 NetTcp ポート共有は [!INCLUDE[indigo1](../../../../includes/indigo1-md.md)]の機能で、同様に複数のネットワーク アプリケーションで 1 つのポートを共有させることができます。NetTcp ポート共有サービスは net.tcp プロトコルを使用して接続を受け入れ、メッセージの送信先アドレスに基づいてメッセージを転送します。  
...
```  
 ...
 ポート共有をサーバー上で有効にするには、<xref:System.ServiceModel.NetTcpBinding> バインディングまたは <xref:System.ServiceModel.Channels.TcpTransportBindingElement> バインディング要素の <xref:System.ServiceModel.NetTcpBinding.PortSharingEnabled%2A> プロパティを設定します。クライアントは、サーバー上で使用されるポート共有の構成内容を知る必要はありません。  
...
4.  ソリューションの C\# 版 または Visual Basic .NET 版をビルドするには、「[Windows Communication Foundation サンプルのビルド](../../../../docs/framework/wcf/samples/building-the-samples.md)」の手順を参照してください。  
  
5.  サンプルを単一コンピュータ構成または複数コンピュータ構成で実行するには、「[Windows Communication Foundation サンプルの実行](../../../../docs/framework/wcf/samples/running-the-samples.md)」の手順を参照してください。このサンプルを実行するための詳細情報については、前述の「サンプルの実行」を参照してください。  
