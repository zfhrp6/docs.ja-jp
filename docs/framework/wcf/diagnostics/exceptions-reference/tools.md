...
|ErrInvalidNamespaceArgument|指定された無効な値が、指定されたオプションに渡されました。コンマ区切りのターゲット名前空間と CLR 名前空間の組み合わせを指定してください。|  
...
|WrnUnableToLoadContractForSGen|コントラクトの種類を読み込み中にエラーが発生しました。このコントラクト用の XmlSerializer 型を生成できません。型と詳細が指定されています。|  
