---
title: "WSTrustChannelFactory および WSTrustChannel | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.reviewer: ""
ms.suite: ""
...
```  
 ...
 <xref:System.ServiceModel.Security.WSTrustChannel.Issue%2A> メソッドで `out` パラメーターを指定することで、クライアント側で検査するために RSTR にアクセスできます。  
...
```  
 ...
 `ChannelFactory` オブジェクトの <xref:System.ServiceModel.ChannelFactory%601.CreateChannelWithIssuedToken%2A> 拡張メソッドは、トークンをアウトオブバンドで取得したこと、発行者に対する通常の WCF 呼び出しを停止して、代わりに取得したトークンを使用して証明書利用者に対して認証を行う必要があることを WIF に示します。  これには、次のような利点があります。  
