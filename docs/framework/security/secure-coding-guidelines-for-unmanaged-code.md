---
title: "アンマネージ コードの安全なコーディングのガイドライン | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
dev_langs: 
  ...
  - "jsharp"
helpviewer_keywords: 
  - "コード セキュリティ, アンマネージ コード"
  - "安全なコーディング, アンマネージ コード"
  - "セキュリティ [.NET Framework], アンマネージ コード"
  - "アンマネージ コード, セキュリティ"
...
---
# アンマネージ コードの安全なコーディングのガイドライン
一部のライブラリ コードは、アンマネージ コードを呼び出す必要があります \(たとえば、Win32 などのネイティブ コード API\)。 これは、マネージ コード用のセキュリティの境界の外部に出ることなので、注意が必要です。 セキュリティ的に中立なコードである場合、コードとそのコードを呼び出すコードは、アンマネージ コードのアクセス許可 \(<xref:System.Security.Permissions.SecurityPermissionFlag> フラグを指定した <xref:System.Security.Permissions.SecurityPermission>\) を持つ必要があります。  
...
## SuppressUnmanagedCodeSecurityAttribute の使用  
