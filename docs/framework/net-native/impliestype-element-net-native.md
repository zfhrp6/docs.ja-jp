---
title: "&lt;ImpliesType&gt; 要素 (.NET ネイティブ) | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
|値|説明|  
|-----------|-----------------|  
|*type_name*|型名。 これによって表される型場合`<ImpliesType>`要素が含まれているのと同じ名前空間にある`<Type>`要素、 *type_name*名前空間なしの型の名前を含めることができます。 それ以外の場合、 *type_name*完全修飾型名を含める必要があります。|  
...
|[<>\>](../../../docs/framework/net-native/method-element-net-native.md)|メソッドにリフレクション ポリシーを適用します。|  
...
```  
 ...
 1 つ以上のシリアライザーに適用される、実際の例を次に示します。 ディレクティブは、何かへのリフレクションとして型指定されている要件をキャプチャする`IList<`*何か*`>`対応へのリフレクションも必要`List<`*何か*`>`せず、アプリケーションごとの注釈の型。  
...
```  
 ...
 `<ImpliesType>` 要素は `<Method>` 要素内にも配置できます。これは、ジェネリック メソッドのインスタンス化が型のインスタンス化へのリフレクションを暗黙的に決定する場合があるためです。 たとえば、ジェネリック メソッド`IEnumerable<T> MakeEnumerable<T>(string` `spelling``, T` `defaultValue``)` 、関連すると共に、特定のライブラリが動的にアクセスする<xref:System.Collections.Generic.List%601>と<xref:System.Array>型です。 これは次のように表すことができます。  
