...
> [!NOTE]
>  いくつかの動作の違い MDI フォームとシングル ドキュメント インターフェイス (SDI) windows と Windows フォームがあります。 `Opacity`プロパティでは MDI 子フォームの外観には影響しません。 さらに、<xref:System.Windows.Forms.Form.CenterToParent%2A>メソッドでは MDI 子フォームの動作には影響しません。  
