...
> [!IMPORTANT]
>  **ツールボックス**、<xref:System.Windows.Forms.SplitContainer>置換の制御、 <xref:System.Windows.Forms.Splitter> Visual Studio の以前のバージョンに存在していたコントロール。 <xref:System.Windows.Forms.SplitContainer> コントロールは <xref:System.Windows.Forms.Splitter> コントロールより優先されます。 <xref:System.Windows.Forms.Splitter>クラスは可能です、 [!INCLUDE[dnprdnshort](../../../../includes/dnprdnshort-md.md)] 、既存のアプリケーションとの互換性を強くお勧めを使用することが、<xref:System.Windows.Forms.SplitContainer>新しいプロジェクトのコントロールです。  
