...
-   名前の一致  
     ...
     **フィールドが** true[!INCLUDE[(vbprvblong]()] の既定値\) の場合、プラットフォーム呼び出しでは、指定した名前だけが検索されます。  たとえば **MessageBox** と指定すると、プラットフォーム呼び出しでは **MessageBox** を検索し、正確に一致する名前が見つからない場合には失敗します。  
