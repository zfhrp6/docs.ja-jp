---
title: "ローダー ETW イベント | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
<a name="top"></a> これらのイベントは、アプリケーションのドメイン、アセンブリ、およびモジュールのロードとアンロードに関連する情報を収集します。  
 ...
 すべてのローダー イベントは、`LoaderKeyword` \(0x8\) キーワードで発生します。`DCStart` および `DCEnd` のイベントは、`StartRundown`\/`EndRundown` が有効になっている `LoaderRundownKeyword` \(0x8\) で発生します。 \(詳細については、「[CLR ETW キーワードおよびレベル](../../../docs/framework/performance/clr-etw-keywords-and-levels.md)」を参照してください\)。  
...
-   名前に"Pdb"が付いているフィールドは、プロファイル セッション中に読み込まれたモジュールに一致する PDB を検索するプロファイリング ツールによって使用できます。 これらのフィールドの値は、読み込まれたモジュールに一致する PDB の位置を特定するためにデバッガーが通常使用する、モジュールの IMAGE\_DIRECTORY\_ENTRY\_DEBUG のセクションに書き込まれたデータに対応します。  
