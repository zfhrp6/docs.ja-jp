---
title: "JIT トレース ETW イベント | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "ETW, JIT トレース イベント (CLR)"
  - "JIT トレース イベント [.NET Framework]"
...
|InlinerName|win:UnicodeString|コンパイラのコード生成の対象であるメソッドの名前。 コンパイラが `InlinerName` への呼び出しを生成するのではなく `MethodBeingCompiledName` へのインライン コードを生成しようとしている場合、これは `MethodBeingCompiledName` とは異なる可能性があります。|  
...
|FailReason|win:UnicodeString|INLINE\_NEVER は、前回のインライン展開の試みで、インライン展開が他の理由により成功しないと判断されたことを意味します。それ以外の場合は、自由形式のテキストです。|  
