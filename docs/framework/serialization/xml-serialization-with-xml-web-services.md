---
title: "XML Web サービスを使用した XML シリアル化 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
helpviewer_keywords: 
  ...
  - "SOAP, XML シリアル化"
  - "XML シリアル化, 属性"
  - "XML シリアル化, XML Web サービス"
  - "XML Web サービス, XML シリアル化"
...
XML シリアル化は、XML Web サービス アーキテクチャで使用される基礎的なトランスポート機構であり、[XmlSerializer](https://msdn.microsoft.com/en-us/library/system.xml.serialization.xmlserializer.aspx) クラスによって実行されます。XML Web サービスによって生成される XML を制御するには、「[XML シリアル化を制御する属性](../../../docs/framework/serialization/attributes-that-control-xml-serialization.md)」および「[エンコード済み SOAP シリアル化を制御する属性](../../../docs/framework/serialization/attributes-that-control-encoded-soap-serialization.md)」の一覧に示されている属性を、XML Web サービスの作成に使用するファイル \(.asmx\) のクラス、戻り値、パラメーター、およびフィールドに適用します。XML Web サービスの作成の詳細については、「[Building XML Web Services Using ASP.NET](http://msdn.microsoft.com/ja-jp/01dfc27c-c68e-4910-a0aa-5e4c2a766b0c)」を参照してください。  
  
## リテラル スタイルとエンコード済みスタイル  
 XML Web サービスによって生成される XML は、「[Customizing SOAP Messages](http://msdn.microsoft.com/ja-jp/1d777288-c0d9-4e6a-b638-f010da031952)」で説明されているリテラルおよびエンコード済みの 2 種類のうち、いずれかの形式を指定できます。このため、XML シリアル化を制御する属性セットは 2 つになります。「[XML シリアル化を制御する属性](../../../docs/framework/serialization/attributes-that-control-xml-serialization.md)」の一覧に示される属性は、リテラル スタイルの XML を制御するように設計されています。一方、「[エンコード済み SOAP シリアル化を制御する属性](../../../docs/framework/serialization/attributes-that-control-encoded-soap-serialization.md)」の一覧に示される属性は、エンコード済みスタイルを制御します。これらの属性を選択的に適用することで、2 つのスタイルのいずれかまたは両方を返すようにアプリケーションを調整できます。さらに、これらの属性は、必要に応じて戻り値やパラメーターにも適用できます。  
  
### 両方のスタイルの使用例  
 XML Web サービスを作成する場合、両方の属性セットをメソッドで使用できます。次のコード例では、`MyService` という名前のクラスに、`MyLiteralMethod` と `MyEncodedMethod` という 2 つの XML Web サービス メソッドが含まれています。いずれのメソッドも、`Order` クラスのインスタンスを返すという同じ機能を実行します。`Order`  クラスでは、[XmlTypeAttribute](frlrfSystemXmlSerializationXmlTypeAttributeClassTopic) 属性と [SoapTypeAttribute](frlrfSystemXmlSerializationSoapTypeAttributeClassTopic) 属性の両方が `OrderID` フィールドに適用され、両方の属性の `ElementName` プロパティには異なる値が設定されます。  
