...
2.  トレース スイッチを構成します。 詳細については、「[トレース スイッチを設定する](#configure)」を参照してください。  
...
## トレース スイッチを構成する  
 ...
 スイッチのインスタンスを作成するときに、*displayName* 引数と *description* 引数の 2 つを指定することで、インスタンスを初期化することもできます。 コンストラクターの *displayName* 引数は、<xref:System.Diagnostics.Switch> クラスのインスタンスの <xref:System.Diagnostics.Switch.DisplayName%2A?displayProperty=fullName> プロパティを設定します。*displayName* は .config ファイルでスイッチを構成するために使用されている名前です。*description* 引数は、スイッチとそれがコントロールするメッセージについて簡単な説明を返します。  
...
1.  トレース スイッチを使用するには、「[トレース スイッチを作成して初期化する](#create)」のセクションに説明されているように、それらを作成してコードに配置する必要があります。  
...
3.  `<configuration>`  タグの後、かつ  `</configuration>`  タグの前に、スイッチを構成する適切な XML を追加します。 次の例では、**DisplayName** プロパティに `DataMessageSwitch` を設定した **BooleanSwitch** と、**DisplayName** プロパティに `TraceLevelSwitch` を設定した **TraceSwitch** を構成しています。  
