...
## 非対称復号化  
 ...
 次の例は、対称キーと IV を表す 2 つのバイト配列の復号化を示しています。  第三者に簡単に送信できる形式で <xref:System.Security.Cryptography.RSACryptoServiceProvider> オブジェクトから非対称の公開キーを抽出する方法については、「[データの暗号化](../../../docs/standard/security/encrypting-data.md)」を参照してください。  
