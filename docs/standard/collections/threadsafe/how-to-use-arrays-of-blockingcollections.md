...
description: "方法: パイプラインでブロッキング コレクションの配列を使用する"
...
次の例は、`TryAddToAny` や `TryTakeFromAny` などの静的メソッドを含む [System.Collections.Concurrent.BlockingCollection&lt;T&gt;](https://docs.microsoft.com/dotnet/core/api/System.Collections.Concurrent.BlockingCollection-1) オブジェクトの配列を使用して、コンポーネント間に高速で柔軟なデータ転送を実装する方法を示します。 
