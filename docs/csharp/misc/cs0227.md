...
アンセーフ コードは \/unsafe でコンパイルした場合のみ有効です。  
 ...
 [!INCLUDE[vs_current_long](../../csharp/misc/includes/vs_current_long_md.md)] で unsafe オプションを設定するには、メイン メニューで **\[プロジェクト\]** をクリックして、**\[ビルド\]** ウィンドウを選択し、\[アンセーフ コードの許可\] チェック ボックスをオンにします。  
