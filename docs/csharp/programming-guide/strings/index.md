...
## 文字列の宣言と初期化  
 ...
 文字列を文字の配列で初期化する場合を除き、文字列オブジェクトの作成に [new](../../../visual-basic/language-reference/operators/new-operator.md) 演算子を使用しないでください。  
...
|[方法: 複数の文字列を連結する](../Topic/How%20to:%20Concatenate%20Multiple%20Strings%20\(C%23%20Programming%20Guide\).md)|`+` 演算子と `Stringbuilder` クラスを使用して、コンパイル時および実行時に文字列を結合する方法について説明します。|  
|[方法 : 文字列を比較する](../Topic/How%20to:%20Compare%20Strings%20\(C%23%20Programming%20Guide\).md)|文字列の序数比較を実行する方法を示します。|  
|[方法: String.Split を使用して文字列を解析する ](../Topic/How%20to:%20Parse%20Strings%20Using%20String.Split%20\(C%23%20Programming%20Guide\).md)|`String.Split` メソッドを使用して文字列を解析するコード例を紹介します。|  
