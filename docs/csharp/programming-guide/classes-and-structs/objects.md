...
## 構造体のインスタンスと. クラスのインスタンス  
 ...
 クラスのインスタンスは、[new 演算子](../../../visual-basic/language-reference/operators/new-operator.md)を使用して作成されます。  次の例では、`Person` が型であり、`person1` と `person 2` がその型のインスタンス \(つまりオブジェクト\) です。  
