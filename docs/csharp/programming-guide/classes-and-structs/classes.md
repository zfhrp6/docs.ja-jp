...
## クラスの継承  
 ...
 C\+\+ とは異なり、C\# のクラスは 1 つの基本クラスから直接継承することしかできません。  ただし、基本クラス自体が別のクラスを継承している場合があるため、1 つのクラスに複数の基本クラスが間接的に継承されることもあります。  さらに、クラスは 1 つ以上のインターフェイスを直接実装できます。  詳細については、「[インターフェイス](../../../visual-basic/reference/command-line-compiler/index.md)」を参照してください。  
