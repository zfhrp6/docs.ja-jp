---
title: "オーバーロードされた演算子 (C# プログラミング ガイド) | Microsoft Docs"
ms.custom: ""
ms.date: "12/03/2016"
...
> [!NOTE]
>  比較演算子をオーバーロードする場合はペアでオーバーロードする必要があります。つまり、`==` をオーバーロードする場合は `!=` もオーバーロードする必要があります。  その反対も同じであり、`<` と `>`、`<=` と `>=` についても同様です。  
...
## 参照  
 ...
 [C\# のオーバーロードされた演算子が常に静的である理由に関するページ](http://go.microsoft.com/fwlink/?LinkId=112383)