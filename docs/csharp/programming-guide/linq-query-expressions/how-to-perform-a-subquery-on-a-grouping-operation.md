---
title: "方法: グループ化操作でサブクエリを実行する (C# プログラミング ガイド) | Microsoft Docs"
ms.custom: ""
ms.date: "12/15/2016"
...
helpviewer_keywords: 
  - "グループ化 LINQ クエリ [C#]"
  - "グループ [C#], LINQ クエリ式"
  - "グループ [C# での LINQ], サブクエリ"
  - "クエリ [C# での LINQ], グループ化"
  - "クエリ [C# での LINQ], サブクエリ"
  - "クエリ式 [C# での LINQ], グループ化"
  - "クエリ式 [C# での LINQ], サブクエリ"
  - "サブクエリ [C#]"
...
ここでは、2 つの異なる方法を使用して、参照元データをグループ単位で並べ替え、各グループに対して個別にサブクエリを実行するクエリを作成します。  ソース要素をグループ化するために各例で使用する基本的な方法は、`newGroup` という名前の新しい*継続*を使用し、`newGroup` に対する新しいサブクエリを生成するというものです。  このサブクエリは、外部クエリによって作成される新しいグループのそれぞれに対して実行されます。  この例では、最終的な出力はグループではなく、匿名型のフラットなシーケンスです。  
 ...
 継続の詳細については、「[into](../../../csharp/language-reference/keywords/into.md)」を参照してください。  次の例では、インメモリ データ構造をデータ ソースとして使用していますが、任意の種類の [!INCLUDE[vbteclinq](../../../csharp/includes/vbteclinq_md.md)] データ ソースで同じ基本原則が当てはまります。  
