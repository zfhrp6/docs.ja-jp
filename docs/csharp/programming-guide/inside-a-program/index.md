---
title: "インサイド C# プログラム | Microsoft Docs"
ms.date: "2015-07-20"
ms.prod: ".net"
ms.technology: 
  - "devlang-csharp"
ms.topic: "article"
dev_langs: 
  - "CSharp"
helpviewer_keywords: 
  - "C# 言語, C# プログラムの概要"
  - "Visual C#, プログラム構造"
ms.assetid: 9551354b-33f0-4e11-bbf0-1a35e3702b22
caps.latest.revision: 14
author: "BillWagner"
ms.author: "wiwagn"
caps.handback.revision: 14
---
# インサイド C# プログラム
ここでは、C\# プログラムの全般的な構造について、標準的な "Hello, World\!" の例を挙げて説明します。  
  
## このセクションの内容  
  
-   [Hello World \-\- 最初のプログラム](../../../csharp/programming-guide/inside-a-program/hello-world-your-first-program.md)  
  
-   [C\# プログラムの一般構造](../../../csharp/programming-guide/inside-a-program/general-structure-of-a-csharp-program.md)  
  
## 関連項目  
  
-   [Getting Started with Visual C\#](../../../csharp/getting-started/getting-started-with-csharp.md)  
  
-   [C\# プログラミング ガイド](../../../csharp/programming-guide/index.md)  
  
-   [C\# リファレンス](../../../csharp/language-reference/index.md)  
  
-   [\<paveover\>C\# Sample Applications](http://msdn.microsoft.com/ja-jp/9a9d7aaa-51d3-4224-b564-95409b0f3e15)  
  
## C\# 言語仕様  
 [!INCLUDE[CSharplangspec](../../../csharp/language-reference/keywords/includes/csharplangspec-md.md)]  
  
## 参照  
 [C\# プログラミング ガイド](../../../csharp/programming-guide/index.md)