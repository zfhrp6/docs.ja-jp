...
次の例では、定数パターンを使用して、特定の日付が、週末か、週の開始日または最終日か、週の途中かを判断します。 つまり、現在の日付の [DateTime.DayOfWeek](xref:System.DateTime.DayOfWeek) プロパティを、@System.DayOfWeek 列挙のメンバーと照合します。 
