...
---
# false 演算子 (C# リファレンス)
オペランドが `false` であることを示す場合には [bool](../../../csharp/language-reference/keywords/bool.md) 値 `true` を返し、それ以外の場合は `false` を返します。  
 ...
 型で `false` 演算子を定義する場合は、[true \(C\# リファレンス\)](../../../csharp/language-reference/keywords/true.md) 演算子も定義する必要があります。  
