...
## 主な演算子  
 ...
 [New](../../../visual-basic/language-reference/operators/new-operator.md) – 型のインスタンス化。  
...
## 単項演算子  
 ...
 [&x](../../../visual-basic/language-reference/operators/and-operator.md) – アドレス。  
...
## シフト演算子  
 ...
 [x \>\> y](../Topic/%3E%3E%20Operator%20\(C%23%20Reference\).md) – ビットを右へシフトします。  左側のオペランドが `int` または `long` の場合、左側のビットには符号ビットが格納されます。  左側のオペランドが `uint` または `ulong` の場合、左側のビットには 0 が格納されます。  
  
## 関係演算子と型検査演算子  
 これらの演算子は、前のセクションより優先順位が低く、次のセクションより優先順位が高くなります。  各演算子をクリックすると、演算子の詳細と例が記載されたページに移動します。  
  
 [x \< y](../Topic/%3C%20Operator%20\(C%23%20Reference\).md) – より小さい \(x が y より小さい場合は true\)。  
 ...
 [x \>\= y](../Topic/%3E=%20Operator%20\(C%23%20Reference\).md) – 以上。  
...
## 論理 AND 演算子  
 ...
 [x & y](../../../visual-basic/language-reference/operators/and-operator.md) – 論理またはビットごとの AND。  通常、整数型および `enum` 型で使用できます。  
  
## 論理 XOR 演算子  
 この演算子は、前のセクションより優先順位が低く、次のセクションより優先順位が高くなります。  各演算子をクリックすると、演算子の詳細と例が記載されたページに移動します。  
  
 [x ^ y](../../../visual-basic/language-reference/operators/xor-operator.md) – 論理またはビットごとの XOR。  通常、整数型と `enum` 型で使用できます。  
...
## 代入演算子とラムダ演算子  
 ...
 [x \*\= y](../../../visual-basic/language-reference/operators/multiplication-assignment-operator.md) – 乗算代入。  `y` の値を `x` の値に乗算した結果を `x` に格納し、新しい値を返します。  
