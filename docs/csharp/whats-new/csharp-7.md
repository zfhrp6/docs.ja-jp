...
---
# <a name="whats-new-in-c-7"></a>C# 7 の新機能

C# 7 では、C# 言語に多くの新機能が追加されます。
...
* [破棄](#discards)
    - 破棄は、割り当てられた値を考慮しない場合は、割り当てで使用される一時、書き込み専用の変数です。 組とユーザー定義型を分解するときにだけでなくを持つメソッドを呼び出すときに特に便利ですが`out`パラメーター。
...
> [!NOTE]
> 組は c# 7、前に使用が効率的なされなかったし、言語のサポートがありませんでした。
> これは、ため、タプル要素として参照のみでした。 `Item1`、`Item2`などです。 C# 7 には、新しいより効率的なタプル型を使用して、組のフィールドのセマンティックの名前を使用する組の言語のサポートが導入されています。

タプルを作成するには、各メンバーを値に割り当てます。

[!code-csharp[UnnamedTuple](../../../samples/snippets/csharp/new-in-7/program.cs#04_UnnamedTuple "Unnamed tuple")]

割り当てがメンバーとして持つタプルを作成している`Item1`と`Item2`と同じ方法で使用できる<xref:System.Tuple>セマンティック名の各タプルのメンバーを提供する組を作成する構文を変更することができます。

[!code-csharp[NamedTuple](../../../samples/snippets/csharp/new-in-7/program.cs#05_NamedTuple "Named tuple")]

`namedLetters` タプルには、`Alpha` と `Beta` と呼ばれるフィールドが含まれています。 これらの名前は、コンパイル時にのみ存在し、実行時にリフレクションを使用して、組を調査する場合などは保持されません。

タプルの割り当てでは、代入の右辺でフィールドの名前を指定することもできます。

[!code-csharp[ImplicitNamedTuple](../../../samples/snippets/csharp/new-in-7/program.cs#06_ImplicitNamedTuple "Implicitly named tuple")]

代入の左辺と右辺の側では、フィールドの名前を指定できます。
...
多くの場合、組を分解するか、メソッドを呼び出すと`out`パラメーター、気にして使用するつもりがない値を持つ変数の定義に強制しています。 C# のサポートが追加*破棄*このシナリオを処理します。 Discard は書き込み専用の変数名を持つ`_`(アンダー スコア文字) です。 すべての 1 つの変数を破棄しようとする値を割り当てることができます。 Discard は未割り当ての変数です。代入ステートメントとは別に、破棄は、コードでは使用できません。

次のシナリオでは、破棄がサポートされています。

* ときに、組またはユーザー定義型の構成を解除します。

* 持つメソッドを呼び出すときに[アウト](../language-reference/keywords/out.md)パラメーター。

* パターン マッチで操作で、[は](../language-reference/keywords/is.md)と[切り替える](../language-reference/keywords/switch.md)ステートメントです。

* 明示的にする場合は、スタンドアロンの識別子として discard としてを代入式の値を識別します。

次の例では定義、`QueryCityDataForYears`を 2 つの異なる年度の都市のデータを格納している 6 組を返すメソッド。 メソッドの呼び出しの例では、メソッドによって返される 2 つの母集団値だけを扱う、ため、タプルをデコンストラクトときに破棄と組で、残りの値を扱います。

[!code-csharp[Tuple-discard](../../../samples/snippets/csharp/programming-guide/deconstructing-tuples/discard-tuple1.cs)]

詳細については、次を参照してください。[破棄](../discards.md)です。
...
`is`パターン式に拡張し、使い慣れた`is`型以外のオブジェクトを照会する演算子です。
...
上のコードでは、`int` の特殊なケースとして `0` のケース、入力がない場合の特殊なケースとして `null` のケースが追加されています。 これは、switch パターン式の重要な新機能の 1 つを示しています。ここでは、`case` 式の順序が重要になります。 `0` のケースは、一般的な `int` のケースの前に記述する必要があります。 そうしないと、一致する最初のパターンは、値が `0` であっても `int` のケースになります。 誤って、match 式は、以降の場合は既に処理されているように並べ替え、コンパイラはフラグを設定して、エラーを生成します。
...
この種類のサイコロをコレクションに追加するには、まずパーセンタイル ダイスを表す型を定義します。
...
C# 言語を誤用から保護するその他の 3 つの規則には、`ref`ローカル変数と返却します。

* 標準的なメソッドの戻り値を割り当てることはできません、`ref`ローカル変数。
    - したがって、`ref int i = sequence.Count();` のようなステートメントは使用できません。
* 有効期間がメソッドの実行期間を超えない変数に `ref` を返すことはできません。
    - つまり、ローカル変数、または同様のスコープを持つ変数への参照を返すことはできません。
* `ref`ローカル変数と返却は、非同期メソッドと共に使用できません。
    - コンパイラは、非同期メソッドが戻るときに、参照先の変数が、最終的な値に設定されてかどうかを知ることはできません。
...
C# 6 では、メンバー関数の[式形式のメンバー](csharp-6.md#expression-bodied-function-members)と読み取り専用プロパティが導入されました。 C# 7 では、式として実装できる許可されたメンバーが拡張されます。 C# 7 では、"*コンストラクター*"、"*ファイナライザー*"、`get` アクセサー、および `set` アクセサーを "*プロパティ*" と "*インデクサー*" に実装できます。 それぞれの例を次のコードに示します。
...
C# では、`throw` は常にステートメントでした。 `throw` は式ではなくステートメントであるため、使用できない C# コンストラクトがありました。 これには、条件式、null 結合式、および一部のラムダ式が含まれます。 式形式のメンバーが追加されたことにより、さらに多くの場所で `throw` 式が役に立つようになりました。 C# 7 では、このようなコンストラクトを記述できるように、"*throw 式*" が導入されています。
...
> [!NOTE]
> NuGet パッケージを追加する必要がある[ `System.Threading.Tasks.Extensions` ](https://www.nuget.org/packages/System.Threading.Tasks.Extensions/)を使用するために、<xref:System.Threading.Tasks.ValueTask%601>型です。
...
数値定数を読み間違えると、コードを初めて読むときにコードを理解するのが難しくなる場合があります。 これは、数値がビット マスクや数値以外の記号として使用されている場合にありがちなことです。 C# 7 では、使用目的に応じて最も読みやすい形式で簡単に数値を記述しできるように、"*バイナリ リテラル*" と "*桁区切り文字*" という 2 つの新機能が導入されました。
