...
プロジェクトのサイズが比較的小さい場合に適しています。  しかし、多くの異なる種類のデータを含み、複数レイヤーになる可能性のある大規模なアプリケーションでは、論理的な整理が必要になります。 その場合はフォルダーを使用します。  このガイドで説明する [NewTypes サンプル プロジェクト](https://github.com/dotnet/docs/tree/master/samples/core/console-apps/NewTypesMsBuild)に従うことも、独自のファイルとフォルダーを作成することもできます。
...
3. `dotnet new -t Xunittest` コマンドを使用して、ディレクトリを初期化します。 ここでは Xunit を前提としますが、`Xunittest` を `Mstest` に置き換えて、MS Test を使用することもできます。
...
1. 以下のものを含む正しい `NewTypesTests.csproj` ファイル:
   ...
   * テスト対象コードに対応する名前空間への参照
   ...
   これは、単に `NewTypesTests` ディレクトリでコマンドラインから `dotnet new -t Xunittest` をビルドし、プロジェクト参照を `NewTypes` プロジェクトに追加することで実行できます。
