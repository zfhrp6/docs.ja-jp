...
最適な機能では、Visual f# のツールを Visual Studio での 1 つは、f# Interactive ウィンドウです。  そのコードを呼び出すし、その結果を対話的に確認できますプロセスでコードを送信することができます。

使用を開始するには、強調表示、`square`コードで定義された関数。  次に、保持、 **Alt**キーを押して**Enter**です。  これは、f# 対話型ウィンドウで、コードを実行します。  F# 対話型ウィンドウ内に次の表示が表示されます。
